...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        prev_h, step_cache = rnn_step_forward(
            x[:, t, :],
            prev_h,
            Wx,
            Wh,
...
def rnn_backward(dh, cache):
    ...
    step_caches = cache
    N, T, H = dh.shape
    x0, prev_h0, Wx, Wh, next_h0 = step_caches[0]
    D = x0.shape[1]
    ...
    for t in reversed(range(T)):
        ...
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dcurrent_h, step_caches[t])
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    loss = torch.nn.functional.cross_entropy(x.permute(0, 2, 1), y, ignore_index=ignore_index, reduction='sum')
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == "rnn":
          self.seq_model = RNN(wordvec_dim, hidden_dim)
        
        elif cell_type == "lstm":
          self.seq_model = LSTM(wordvec_dim, hidden_dim)

        else:
          self.attn_proj = nn.Conv2d(input_dim, hidden_dim, kernel_size=1)
          self.seq_model = AttentionLSTM(wordvec_dim, hidden_dim)
    ...
    def forward(self, images, captions):
        ...
        wordvecs = self.word_embed(captions_in)

        if self.cell_type == "attn":
            A = self.attn_proj(features)          
            hidden = self.seq_model(wordvecs, A)  
        
        else:
            features = features.mean(dim=(2, 3)) 
            h0 = self.feat_proj(features)         
            hidden = self.seq_model(wordvecs, h0)  

        scores = self.out_proj(hidden)             
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            ...
            h = A.mean(dim=(2, 3))              
            c = h.clone()
            attn_weights_all = images.new_zeros(N, max_length, A.shape[2], A.shape[3]).float()
        ...
        current_word = torch.full(
            (N,), self._start, dtype=torch.long, device=images.device
        )

        for t in range(max_length):
            wordvec = self.word_embed(current_word)  
            ...
            if self.cell_type == "rnn":
                ...
                h, c = self.seq_model.step_forward(wordvec, h, c)

            else:
                attn, attn_weights = dot_product_attention(h, A)
                h, c = self.seq_model.step_forward(wordvec, h, c, attn)
...
def dot_product_attention(prev_h, A):
    ...
    attn = (A_flat * attn_weights.view(N, 1, D_a * D_a)).sum(dim=2)
