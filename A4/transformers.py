...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        dots = torch.zeros(K, dtype=query.dtype)
        
        for j in range(K):
            dots[j] = torch.dot(query[i], key[j]) / (M ** 0.5)
        
        weights = F.softmax(dots, dim=0)
        out[i] = weights @ value
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            ...
            dots = (query[n, i] @ key[n].T) / (M ** 0.5)   # shape (K,)
            weights = F.softmax(dots, dim=0)
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        weights = weights.masked_fill(mask.to(weights.device), -1e9)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.linear = nn.Linear(concat_dim, dim_in)
    ...
    def forward(
        ...
        concat = torch.cat(head_outputs, dim=-1)   #(N, K, num_heads * dim_out)
        y = self.linear(concat)                    #(N, K, dim_in)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.ff1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        c1 = (6 / (inp_dim + hidden_dim_feedforward)) ** 0.5
        nn.init.uniform_(self.ff1.weight, -c1, c1)
 
        self.ff2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        c2 = (6 / (hidden_dim_feedforward + inp_dim)) ** 0.5
        nn.init.uniform_(self.ff2.weight, -c2, c2)      
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.MultiHeadBlock = MultiHeadAttention(num_heads, emb_dim, emb_dim)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        if mask is None:
            ...
            mask = torch.triu(torch.ones(K, K, dtype=torch.bool, device=dec_inp.device), diagonal=1).unsqueeze(0).expand(N, -1, -1)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = pos.unsqueeze(1).expand(K, M)                 # shape (K, M)
    y = y.unsqueeze(0)                                # shape (1, K, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    div_term = torch.pow(10000.0,torch.arange(0, M, 2, dtype=torch.float32) / M)                                                                  # (M//2,)
    
    pe[:, 0::2] = torch.sin(pos / div_term)
    pe[:, 1::2] = torch.cos(pos / div_term[:M // 2])
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        enc_out = self.encoder(q_emb_inp)                         # (N, K, M)
        mask = get_subsequent_mask(ans_b[:, :-1])                 # (N, O-1, O-1)
        dec_out = self.decoder(a_emb_inp, enc_out, mask)          # (N, O-1, vocab_len)
